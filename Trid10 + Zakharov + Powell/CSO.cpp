...
int main(int argc, char* argv[])
{
	...
	int i_Cycle[i_FuncNum]={2 , 2, 2};		// Defines the total cycle for runing this algorithm. 
	...
	int i_Dim[i_FuncNum]={10 ,2 , 4};			// Defines the dimension of the benchmark functions.
	...
	int i_PSize[i_FuncNum]={30 , 30 , 30};		// Defines the population size used to run the CSO algorithm.

  double d_InitL[i_FuncNum]={-100 , -5 , -4};       // Defines the left border of the initial ranges.
  double d_InitR[i_FuncNum]={100 , 10 , 5};       // Defines the right border of the initial ranges.
  double d_MaxVel[i_FuncNum]={100 , 10 , 5};     // Defines the maximum velocity of the cats.
  ...
  timer=mytimer_create();
	...
	for (int k=0; k<i_Iteration[(i-1)]; k++){
		w[k]=0.45+(((double)(i_Iteration[(i-1)]-k))/((double)(2*i_Iteration[(i-1)])));	
	}
