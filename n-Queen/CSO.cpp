...
void CSO_Algorithm(int i_Cycle,int i_Iteration,int i_PSize,int i_Dim,double d_InitL,double d_InitR,double d_MaxVel,bool b_Maximize,int i_FN,mytimer *timer,double d_avefns[],double d_avetime[])
{
  ...
  double d_Start, d_Stop;
	...
	for(int co = 0 ; co < i_Dim ; co++){
		printf("%d " , t_cat[i_PSize].sol.sortedOrder[co]);
	}
	printf("\n Hit is : \n");
	for(int co = 0 ; co < i_Dim ; co++){
		for(int jp = 0 ; jp < i_Dim ; jp++)
			printf("%d " , t_cat[i_PSize].sol.hit[co][jp]);
		printf("\n");
	}
	printf("\n");
...
int main(int argc, char* argv[])
{
	...
	int i_Iteration[i_FuncNum]={1000}; // Defines the total iteration number in one cycle.
	...
	int i_Dim[i_FuncNum]={24};			// Defines the dimension of the benchmark functions.
	...
	int i_PSize[i_FuncNum]={20};		// Defines the population size used to run the CSO algorithm.
