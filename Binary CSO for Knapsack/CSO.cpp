...
void CSO_Algorithm(int i_Cycle,int i_Iteration,int i_PSize,int i_Dim,double d_InitL,double d_InitR,double d_MaxVel,bool b_Maximize,int i_FN,mytimer *timer,double d_avefns[],double d_avetime[])
{
  ...
  double d_Start, d_Stop;
	...
	printf("best cost is : %f \n" , t_cat[i_PSize].sol.cost);
	printf("best SumVX is : %f \n" , t_cat[i_PSize].sol.sumVX);
	printf("best SumWX is : %f \n" , t_cat[i_PSize].sol.sumWX);

	printf("Values is : \n");
	for(int co = 0 ; co < i_Dim ; co++){
		printf("%d " , t_cat[i_PSize].sol.values[co]);
	}
	printf("\n weight is : \n");
	for(int co = 0 ; co < i_Dim ; co++){
		printf("%d " , t_cat[i_PSize].sol.weights[co]);
	}
...
int main(int argc, char* argv[])
{
	...
	int i_Cycle[i_FuncNum]={1};		// Defines the total cycle for runing this algorithm. 
	...
	int i_Iteration[i_FuncNum]={2000}; // Defines the total iteration number in one cycle.
	...
	int i_Dim[i_FuncNum]={20};			// Defines the dimension of the benchmark functions.
	...
	int i_PSize[i_FuncNum]={30};		// Defines the population size used to run the CSO algorithm.

  double d_InitL[i_FuncNum]={0};       // Defines the left border of the initial ranges.
  double d_InitR[i_FuncNum]={1};       // Defines the right border of the initial ranges.
  double d_MaxVel[i_FuncNum]={1};     // Defines the maximum velocity of the cats.
  bool b_Maximize[i_FuncNum]={0};                // Defines the goal of optimization. Set "1" to let CSO find the near maximum solution, and set "0" to find the minimum.
