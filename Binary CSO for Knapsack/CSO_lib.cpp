...
//((Rand()*(d_InitR-d_InitL))+d_InitL); // Set the coordinate of the cat randomly in the initial range.
  ...
  sol tempSol =Benchmark(i_FN,i_Dim,d_copy);  // Calculate the fitness value of the randomly generated near best solution.
  ...
  /* Pick i_NTracing cats and set their STFlage to be activated - Start */
	t_cat[i_PSize].val = tempSol.cost;
	t_cat[i_PSize].sol.cost = tempSol.cost;
	t_cat[i_PSize].sol.hit = new int*[i_Dim];
	for(j = 0 ; j < i_Dim ; j++)
		t_cat[i_PSize].sol.hit[j] = new int[i_Dim];
	for( i = 0 ; i < i_Dim ; i++){
		for( j = 0 ; j < i_Dim ; j++)
			t_cat[i_PSize].sol.hit[i][j] = tempSol.hit[i][j];
	}	

	t_cat[i_PSize].sol.sortedOrder = new int[i_Dim];
	for( i = 0 ; i < i_Dim ; i++){
		t_cat[i_PSize].sol.sortedOrder[i] = tempSol.sortedOrder[i];
	}	
...
//		printf("4 \n");
    sol tempSol;
    tempSol=Benchmark(i_FN,i_Dim,d_copy);  // Calculate the fitness value of the ith cat.
	t_cat[i].val = tempSol.cost;
...
//			printf("6 \n");
	...
	for( k = 0 ; k < i_Dim ; k++){
		for( j = 0 ; j < i_Dim ; j++)
			t_cat[i_PSize].sol.hit[k][j] = tempSol.hit[k][j];
	}	
//			printf("7 \n");
	t_cat[i_PSize].sol.sortedOrder = new int[i_Dim];
//		printf("71 \n");
	for( k = 0 ; k < i_Dim ; k++){
//				printf("72 %d \n" , i);
		t_cat[i_PSize].sol.sortedOrder[k] = tempSol.sortedOrder[k];
	}	
//			printf("73 \n");
        for(j=0;j<i_Dim;j++)
        {
...
//		printf("i is : %d j is : %d penalty flag set to true %d \n" , i , j , d_Seeking[((i*i_Dim)+j)]);
    ...
    d_cpfns[i]=Benchmark(i_FN,i_Dim,d_copy).cost;      // Calculate the fitness value of the seeking spot.
